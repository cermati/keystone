...
exports = module.exports = function(req, res) {
	...
	var renderView = function() {
		...
		var link_to = function(params) {
			...
			return '/internal/cms/keystone/' + req.list.path + (p ? '/' + p : '') + (params ? '?' + params : '');
		};
		...
		query.exec(function(err, items) {
			...
			if (req.query.search && items.total === 1 && items.results.length === 1) {
				return res.redirect('/internal/cms/keystone/' + req.list.path + '/' + items.results[0].id);
			}

			var download_link = '/internal/cms/keystone/download/' + req.list.path;
			var downloadParams = {};
	...
	if ('update' in req.query) {
		...
		(function() {
			...
			req.list.updateAll(data, function(err) {
				...
				res.redirect('/internal/cms/keystone/' + req.list.path);
			});
		})();
		...
		req.list.model.findById(req.query['delete']).exec(function (err, item) { //eslint-disable-line dot-notation
			if (err || !item) return res.redirect('/internal/cms/keystone/' + req.list.path);
