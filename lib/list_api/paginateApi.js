					...
					_.forEach(searchQueryRegex, function(value, key) {
						if (key != '$or') {
							if (value instanceof RegExp) {
								var stringValue = _.replace(value.source, /\\/g, '');
								stringValue = encodeURIComponent(stringValue);
								model._params[key] = stringValue;
							}
							else {
								if (value instanceof Object) {
									_.forEach(value, function(valObj, keyObj) {
										if (keyObj === '$ne') {
											model._params[key] = !Boolean(valObj);
										}
									});
								}
								else {
									var stringValue = _.replace(value, /\\/g, '');
									stringValue = encodeURIComponent(stringValue);
									model._params[key] = stringValue;
								}
							}
				...
				.then(function (res) {
					...
					return Bluebird.props({
						...
						last: Bluebird
							.resolve()
							.then(() => options.apiDetails.read.getResponseData(responseBody))
							.then((items) => (currentPage - 1 ) * resultsPerPage + _.size(items))
					});
				})
				.then((result) => {
