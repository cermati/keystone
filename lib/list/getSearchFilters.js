 ...
 */
function getSearchFilters (search, add, searchWithRegex) {
	var filters = {};
	var list = this;
	...
	if (search.length) {
		...
		searchFields.forEach(function (path) {
			...
			if (field && field.type === 'name') {
				var first = {};
				first[field.paths.first] = (searchWithRegex) ? splitSearchRx : search;
				var last = {};
				last[field.paths.last] = (searchWithRegex) ? splitSearchRx : search;
				searchFilter = {};
				searchFilter.$or = [first, last];
				searchFilters.push(searchFilter);
			} else {
				searchFilter = {};
				searchFilter[path] = (searchWithRegex) ? searchRx : search;
				searchFilters.push(searchFilter);
			}
		});
		...
		if (list.autokey) {
			searchFilter = {};
			searchFilter[list.autokey.path] = (searchWithRegex) ? searchRx : search;
			searchFilters.push(searchFilter);
		}
