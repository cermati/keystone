...
function routes(app) {
	...
	function dropNav() {
		return function(req, res, next) {
			if (req.user.activeBlockedGroups) {
				...
				if (_.includes(dropList, req.params.list)) {
	...
	app.all('/keystone', dropNav(), require('../../admin/routes/views/home'));
	...
	app.all('/keystone/api/:list/:action(autocomplete|order|create|fetch)', initList(), dropNav(), require('../../admin/api/list'));
	app.post('/keystone/api/:list/delete', initList(), dropNav(), require('../../admin/api/list/delete'));
	app.get('/keystone/api/:list', initList(), dropNav(), require('../../admin/api/list/get'));
	app.get('/keystone/api/:list/:id', initList(), dropNav(), function(req,res,next){
	...
	app.post('/keystone/api/:list/:id/delete', initList(), dropNav(), require('../../admin/api/item/delete'));

	//Prevent access download page if use api
	app.all('/keystone/download/:list', initList(), dropNav(), (req,res,next) => {
		if (req.list.options.useApi){
	...
	// Admin-UI API
	app.all('/keystone/:list', initList(true), dropNav(), require('../../admin/routes/views/list'));
	app.get('/keystone/:list/create', initList(true), dropNav(), require('../../admin/routes/views/itemApi'));
	app.all('/keystone/:list/:item', initList(true), dropNav(), function (req, res, next) {
		if (req.list.options.useApi) {
