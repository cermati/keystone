...
function routes(app) {
	...
	function blockListAccess(req, res, next) {
		if (req.user.activeGroups && req.user.dropList && req.user.dropListBySectionKey && req.params.list) {
			const dropListBySectionKey = req.user.dropListBySectionKey;
			// Find group name of a list that want to be accessed via user request
			// Complexity O(n*m) where n is number of groups and m is number of list in a group
			const groups = _.filter(keystone.originalNav.sections, (section) => {
				const camelCasedSectionByLabel = _.keyBy(section.lists, (o) => {
					return _.camelCase(o.label);
				});
				return Boolean(camelCasedSectionByLabel[_.camelCase(req.params.list)]);
			});

			if (_.size(groups)) {
				// Only use first segment at index 0 is enough
				// O(1) via dropListBySectionKey lookup
				if (Boolean(dropListBySectionKey[groups[0].key])) {
	...
	// Need initList or initBlockedList middleware
	function filterNav(req, res, next) {
		if (req.user.activeGroups && req.user.dropList && req.user.dropListBySectionKey) {
			const nav = keystone.originalNav;
			const dropListBySectionKey = req.user.dropListBySectionKey;
			const dropList = req.user.dropList;

			const filteredNav = {
				sections: _.filter(nav.sections, (section) => {
					// Lookup from dropListBySectionKey with complexity O(1)
					return !Boolean(dropListBySectionKey[section.key]);
				}),
				by: {
					list: _.omit(nav.by.list, req.dropList),
					section: _.omit(nav.by.section, req.dropList)
				}
			};
	...
	function initBlockedList(req, res, next) {
		// If groups not found then skip
		if (!req.user || !_.hasIn(req.user, 'groups')) {
			next();
			return;
		}
		...
		userModel.findOne({_id: req.user._id})
			...
			.then((groups) => {
				const dropList = _.uniq(groups.reduce((acc, group) => {
					if (!group.active) {
						return acc;
					}
					return acc.concat(group.blockedGroups);
				}, []));

				const dropListBySectionKey = dropList.reduce((acc, dropKey) => {
					return _.set(acc, dropKey, true);
				}, {});

				req.user.dropList = dropList;
				req.user.dropListBySectionKey = dropListBySectionKey;
				next();
			})
			.catch((err) => {
	...
	app.all('/keystone', initBlockedList, filterNav, require('../../admin/routes/views/home'));
	...
	// Admin-UI API
	app.all('/keystone/:list', initList(true), filterNav, blockListAccess, require('../../admin/routes/views/list'));
	app.get('/keystone/:list/create', initList(true), filterNav, blockListAccess, require('../../admin/routes/views/itemApi'));
	app.all('/keystone/:list/:item', initList(true), filterNav, blockListAccess, function (req, res, next) {
		if (req.list.options.useApi) {
