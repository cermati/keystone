...
function routes(app) {
	...
	// bounce user if not authorized to access current list
	...
	// dropping nav that include in blocked list of current user
	function dropNav() {
		return function(req, res, next) {
	...
	function initList(respectHiddenOption) {
		return function(req, res, next) {
			...
			// Collect blocked groups of user
			userModel.findOne({_id: req.user._id})
				...
				.then((groups) => {
					...
					_.forEach(groups, (group) => {
						console.log(group.blockedGroups);
						if (group.active) {
				...
				.then((blockedGroups) => {
					// attach to request so we don't need re-query again
