...
function routes(app) {
	...
	function blockListAccess(req, res, next) {
		if (req.user.activeGroups && req.user.dropList && req.user.dropListMapper && req.params.list) {
			const dropListMapper = req.user.dropListMapper;
			...
			const groups = _.filter(keystone.originalNav.sections, (section) => {
				const camelCasedSectionMapper = _.keyBy(section.lists, (o) => {
					return _.camelCase(o.label);
				});
				// Lookup from camelCasedSectionMapper object with O(1) complexity
				return Boolean(camelCasedSectionMapper[_.camelCase(req.params.list)]);
			});

			if (_.size(groups)) {
				// Only use first segment at index 0 is enough
				// O(1) via dropListMapper lookup
				if (Boolean(dropListMapper[groups[0].key])) {
	...
	// Need initList or initBlockedList middleware
	function dropNav(req, res, next) {
		if (req.user.activeGroups && req.user.dropList && req.user.dropListMapper) {
			...
			const filteredNav = {
				sections: _.filter(nav.sections, (section) => {
					// Lookup from dropListMapper with complexity O(1)
					return !Boolean(dropListMapper[section.key]);
				}),
				by: {
	...
	function initBlockedList(req, res, next) {
		...
		userModel.findOne({_id: req.user._id})
			...
			.then((groups) => {
				...
				const dropListMapper = dropList.reduce((acc, dropKey) => {
					return _.set(acc, dropKey, true);
				}, {});

				req.user.dropList = dropList;
				req.user.dropListMapper = dropListMapper;
				next();
			})
			.catch((err) => {
